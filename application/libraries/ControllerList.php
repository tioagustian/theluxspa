    ...
    private function setControllers() {
        ...
        foreach(glob(APPPATH . 'modules/*') as $controller) {
            ...
            // if(is_dir($controller)) {
                ...
                $filename = basename($controller, EXT);

                // Loop through the subdirectory
                // foreach(glob(APPPATH . 'modules/'.$controller.'/*') as $subdircontroller) {
                    ...
                    if(!class_exists($filename)) {
                        $this->CI->load->file($controller.'/controllers'.'/'.$filename.'.php');
                    }
                    // Add the controllername to the array with its methods
                    $aMethods = get_class_methods($controller);
                    $aUserMethods = array();
                    foreach($aMethods as $method) {
                        if($method != '__construct' && $method != 'get_instance' && $method != $filename) {
                            $aUserMethods[] = $method;
                        }
                    }
                    $this->setControllerMethods($filename, $aUserMethods);                                      
                // }
            }
