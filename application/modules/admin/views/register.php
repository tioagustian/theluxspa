...
<div class="pace-activity"></div></div>
    <section class="login-block">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-12">
                    <form class="md-float-material form-material" id="form" url="<?= $url;?>">
...
<![endif]-->
    ...
    <script type="text/javascript">
            ...
            $('#info').fadeIn('500', function () {
                ...
                for (var i = 0; i < input.length; i++) {
                    ...
                    if (typeof allowEmpty[name] == 'undefined') {
                        if (isNull(value)) {
                            valid = cancel(name, value, input, el)
                        } else {
                            if (name == 'username') {
                                valid = usernameCheck(input, el)
                            }
                            if (name == 'email') {
                                valid = emailCheck(input, el)
                            }
                        }
                    }
                    data[name] = value
        ...
        function usernameCheck(input, el) {
            var name = el.name
            var value = el.value
            var url = document.querySelector('form#form').getAttribute('url') + '/usernameCheck'
            var message = 'Username '+ value  + ' telah digunakan. Silahkan gunakan yang lain'
            jQuery.ajax({
                url: url,
                type: 'POST',
                dataType: 'JSON',
                ...
                success: function (result) {
                    if (result.status) {
                        cancel(name, value, input, el, message)
                        this.act = false
                    } else {
                        clearMessage(el)
                        this.act = true
        ...
        function emailCheck(input, el) {
            var name = el.name
            var value = el.value
            var url = document.querySelector('form#form').getAttribute('url') + '/emailCheck'
            var message = 'Email '+ value  + ' telah terdaftar. Silahkan gunakan yang lain'
            jQuery.ajax({
                url: url,
                type: 'POST',
                dataType: 'JSON',
        ...
        function cancel(name, value, input, el, message) {
            ...
            return this.act = false
        } 

        function send(data) {
            var url = document.querySelector('form#form').getAttribute('url') + '/post'
            jQuery.ajax({
                url: url,
                type: 'POST',
                dataType: 'JSON',
        ...
        function showResult(success) {
            if (success) {
                $('.preloader4').fadeOut('500', function() {
                    $('#success').fadeIn('500')
                })
            } else {
                $('#info').fadeOut('500', function () {
